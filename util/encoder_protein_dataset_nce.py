...
class EncodedProteinDataset_nce(Dataset):
    ...
    def __init__(self, msa_folder, encodings_folder, noise=0.0, max_msas=None, max_seqs=None):
        ...
        for numseq_file in filter(lambda file: file.endswith('pt') and (not file.startswith('pi_')), os.listdir(msa_folder)):
            ...
            pi_path = os.path.join(msa_folder, 'pi_'+numseq_file)
...
def collate_fn_new(batch, q, batch_msa_size):
    ...
    ## Remember padding is due to glueing different proteins of different length together
    pi_s = pad_sequence(pi_s, batch_first=True, padding_value=q)
    ...
    return msas, encodings, pi_s, padding_mask
